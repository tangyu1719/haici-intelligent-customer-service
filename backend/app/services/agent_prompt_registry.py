...
AGENT_CATALOG: dict[str, dict[str, Any]] = {
    # ── VLM 图片理解（按职责合并为一个入口，内部按图片类型自动路由子Agent） ──
    "vlm_image_agent": {
        "label": "VLM 图片理解",
        "group": "multimodal_image",
        "kind": "vlm",
        "task_type": "vlm",
        "variables": ["doc_context", "image_type", "title_hint", "ocr_text"],
        "hint": "视觉语言模型直接理解图片内容，自动按类型路由到对应描述模板（菜单/设计/流程/图表/接口/通用）",
        "internal_agents": [
            "image_type_classifier_agent",
            "image_describe_ui_menu_agent",
            "image_describe_ui_design_agent",
