...
> **日期**：2026-06-12  
> **状态**：已裁决 · 实施中  
> **PRD**：`HaiCi笔试_AI 智能客服系统.md`（只读，禁止修改）
...
| implementation | 登录/Casbin/日志/会话 **已落地**；UI v0.5 密码主登录 |
| access_token | 30 分钟（Owner 确认 OK） |
| refresh_token | 30 天 + 滑动续期（Owner 确认 OK） |
...
自增 ID 易泄露业务量，**禁止**对外使用 `id`。采用 **不可逆 Hash 映射** 生成 10 位数字对外码（比裸 UUID 更贴近电商会员号习惯，且不可推测下一号）。
...
见 v0.3 §4：智能客服（对话/会话历史）· 知识库 · 个人中心 · 运维管理（admin）。
